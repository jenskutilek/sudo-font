SOURCES=$(shell python3.13 scripts/read-config.py --sources )
FAMILY=$(shell python3.13 scripts/read-config.py --family )
...
help:
	...
	@echo "  make test-gf:   Tests the fonts with fontbakery"
...
customize: venv
	. venv/bin/activate; python3.13 scripts/customize.py
...
%.png: %.py build.stamp
	. venv/bin/activate; python3.13 $< --output $@
